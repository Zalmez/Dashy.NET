...
/// <summary>
/// API-side edit lock service that manages edit locks for dashboard configurations.
/// This ensures only one user can edit a dashboard configuration at a time.
/// </summary>
public class ApiEditLockService
{
    private readonly ConcurrentDictionary<int, EditLockInfo> _dashboardLocks = new();
    private readonly ILogger<ApiEditLockService> _logger;
    private readonly Timer _cleanupTimer;
    private static readonly TimeSpan LockTimeout = TimeSpan.FromMinutes(5);

    public ApiEditLockService(ILogger<ApiEditLockService> logger)
    {
        _logger = logger;
    ...
    /// <summary>
    /// Converts internal EditLockInfo to DTO.
    /// </summary>
    public EditLockDto? ToDto(EditLockInfo? lockInfo)
    {
        if (lockInfo == null) return null;

        return new EditLockDto
        {
            UserId = lockInfo.UserId,
            UserName = lockInfo.UserName,
            LockedAt = lockInfo.LockedAt,
            LastActivity = lockInfo.LastActivity
        };
    }
